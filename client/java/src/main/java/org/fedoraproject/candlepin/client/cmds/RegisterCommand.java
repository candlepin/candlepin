...
public class RegisterCommand extends BaseCommand {
    ...
    public Options getOptions() {
        ...
        opts.addOption("c", "consumerid", true, "Register to an Existing consumer");
        opts.addOption("a", "autosubscribe", true, "Automatically subscribe this system to compatible subscriptions.");
    ...
    public void execute(CommandLine cmdLine) {
        ...
        String consumerId = cmdLine.getOptionValue("c");
        boolean force = cmdLine.hasOption("f");
        boolean bothUserPassPresent = isNotEmpty(username) && isNotEmpty(password);
        if (!bothUserPassPresent && consumerId == null) {
            System.err.println("Error: username and password or consumerid are required to register,try --help.\n");
            return;
        }else if(bothUserPassPresent && consumerId != null){
        	System.err.println("Error1: username and password or consumerid are required, not both. try --help.\n");
        	return;
        }
        CandlepinConsumerClient client = this.getClient();
        
        if (client.isRegistered() && !force) {
            System.out.println("This system is already registered. Use --force to override");
            return;
        }
        //register based on client id. Go ahead with invocation
        if(consumerId != null){
        	OperationResult reason = this.client.registerExistingCustomerWithId(consumerId);
	...
	private void printMsgs(String consumerId, OperationResult reason) {
		switch(reason){
		case  NOT_A_FAILURE:
			System.out.println("Registered with UUID: " + this.client.getUUID());
			break;
		case ERROR_WHILE_SAVING_CERTIFICATES:
			System.err.println("Error while trying to save certificate and keys to disk.");
			break;
		case INVALID_UUID:
			System.err.println("Unable to register with consumer " + consumerId);
			break;
		case UNKNOWN:
			System.err.println("Registration failed due to unknown reason. Please try again.");
			break;
		}
	}
