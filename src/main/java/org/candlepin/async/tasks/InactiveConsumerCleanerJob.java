...
public class InactiveConsumerCleanerJob implements AsyncJob {
    ...
    private <T> int deleteInBatches(List<T> items, int batchSize, String entityDescription,
        ...
        for (List<T> batch : Iterables.partition(items, batchSize)) {
            ...
            deletedCount += this.transactionalWithEventRollback(() -> deletionFunction.apply(batch));
        }

        return deletedCount;
    }

    private <T> T transactionalWithEventRollback(Supplier<T> task) {
        return new Transactional(this.consumerCurator.getEntityManager())
