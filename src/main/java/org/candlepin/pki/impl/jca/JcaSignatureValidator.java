...
public class JcaSignatureValidator implements SignatureValidator {
    ...
    public SignatureValidator withAdditionalCertificates(Collection<X509Certificate> certs) {
        ...
        // This is so dumb, but .contains isn't safe to use for nulls thanks to very very bad decisions at
        // the JDK level.
        if (certs.stream().anyMatch(Objects::isNull)) {
            throw new IllegalArgumentException("certs contains null certificates");
        }

        this.additionalCerts.addAll(certs);
