...
public class SCACertificateGenerator {
     ...
     *  if there is an issue persisting the X509 certificate due to a concurrent request causing a database
     ...
     *  consumer is not v3 cert capable
     */
    public SCACertificate generate(Consumer consumer) throws ConcurrentCertificateCreationException {
        if (consumer == null) {
            throw new IllegalArgumentException("consumer is null");
        }
    ...
    private SCACertificate getCertificate(Owner owner, Consumer consumer, List<Environment> environments)
        ...
        if (scaCertificate == null) {
            ...
            try {
                ...
                throw new ConcurrentCertificateCreationException(e);
            }

            consumer.setContentAccessCert(scaCertificate);
        }

        int threshold = configuration.getInt(ConfigProperties.SCA_X509_CERT_EXPIRY_THRESHOLD);
        Date regenerationCutoff = Date.from(Instant.now()
            .plus(Long.valueOf(threshold), ChronoUnit.DAYS));
    ...
    private ContentAccessPayload getContentAccessPayload(Owner owner, Consumer consumer,
        List<Environment> environments) throws ConcurrentCertificateCreationException {

        String payloadKey = new ContentAccessPayloadKeyBuilder(consumer)
            .setEnvironments(environments)
            .build();

        ContentAccessPayload container = contentAccessPayloadCurator
            .getContentAccessPayload(owner.getId(), payloadKey);

        if (container == null || isPayloadExpired(container, owner, environments)) {
            log.info("Building content access payload for: {}, {}", owner.getKey(), payloadKey);

            Date timestamp = new Date();
            String payload = buildContentAccessPayload(owner, consumer, environments);
            if (container == null) {
                ...
                try {
                    contentAccessPayloadCurator.create(container);
                }
                catch (EntityExistsException e) {
                    throw new ConcurrentCertificateCreationException(e);
                }
