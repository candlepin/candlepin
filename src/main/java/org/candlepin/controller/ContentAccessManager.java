...
public class ContentAccessManager {
            ...
            ContentAccessCertificate result = this.consumerCurator.<ContentAccessCertificate>transactional()
                ...
                .execute(args -> {
                    ...
                    return existing == null ?
                        this.contentAccessCertificateGenerator.createCertificate(consumer, owner) :
                        this.contentAccessCertificateGenerator.updateCertificate(consumer, owner, existing);
                });
    ...
    private String resolveContentAccessListValue(String value, boolean resolveNull) {
        if (value == null) {
            return resolveNull ? ContentAccessMode.getListDefaultDatabaseValue() : null;
        }

        if (value.isEmpty()) {
            return ContentAccessMode.getListDefaultDatabaseValue();
        }
    ...
    public AnonymousContentAccessCertificate getCertificate(AnonymousCloudConsumer consumer) {
        if (standalone) {
            throw new UnsupportedOperationException("""
                Cannot retrieve or create anonymous content access certificate in standalone mode""");
        }
