...
import org.candlepin.config.AttributeValidatorConfig;
...
public class AttributeValidator extends PropertyValidator {
    ...
    @Inject
    public AttributeValidator(AttributeValidatorConfig config, Provider<I18n> i18nProvider) {
        Set<String> attributes;

        // Add integer attributes...
        attributes = config.intAttributes();
        if (attributes != null) {
        ...
        attributes = config.positiveIntAttributes();
        if (attributes != null) {
            for (String key : attributes) {
        ...
        // Add long attributes...
        attributes = config.longAttributes();
        if (attributes != null) {
            for (String key : attributes) {
        ...
        attributes = config.positiveLongAttributes();
        if (attributes != null) {
            for (String key : attributes) {
        ...
        attributes = config.boolAttributes();
        if (attributes != null) {
            for (String key : attributes) {
