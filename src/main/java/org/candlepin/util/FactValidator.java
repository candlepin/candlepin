...
import org.candlepin.config.FactValidatorConfig;
...
public class FactValidator extends PropertyValidator {
    ...
    @Inject
    public FactValidator(FactValidatorConfig config, Provider<I18n> i18nProvider) {
        Set<String> attributes;

        // Add integer attributes...
        attributes = config.intFacts();
        if (attributes != null) {
        ...
        attributes = config.positiveIntFacts();
        if (attributes != null) {
            for (String key : attributes) {
