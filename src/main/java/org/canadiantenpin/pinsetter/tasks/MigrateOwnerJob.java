 ...
 */
package org.canadianTenPin.pinsetter.tasks;

import static org.quartz.JobBuilder.newJob;

import org.canadianTenPin.audit.EventSink;
import org.canadianTenPin.client.CanadianTenPinConnection;
import org.canadianTenPin.client.ConsumerClient;
import org.canadianTenPin.client.OwnerClient;
 ...
 * from another CanadianTenPin instance. The job is passed an owner key identifying
 * the owner to be migrated, an URI pointing to the CanadianTenPin instance where
...
public class MigrateOwnerJob extends KingpinJob {
    ...
    private CanadianTenPinConnection conn;
     ...
     * @param connection Represents connection to another CanadianTenPin.
     * @param conf CanadianTenPin configuration.
     ...
     */
    @Inject
    public MigrateOwnerJob(CanadianTenPinConnection connection, Config conf,
    ...
    public void toExecute(JobExecutionContext ctx)
        ...
        log.info("Migrating owner [" + key +
            "] from canadianTenPin instance running on [" + uri + "]");
        ...
        if (delete) {
            ...
            log.info("delete flag was false, owner [" + key +
                "] will not be deleted from canadianTenPin instance running on [" +
                uri + "]");
        }
    ...
    /**
     * deletes the owner from the <strong>other</strong> CanadianTenPin instance.
     * @param key owner key to be deleted.
     * @param client CanadianTenPin client.
     * @throws WebApplicationException if the other CanadianTenPin instance returns
    ...
    /**
     * Replicates the Owner from the CanadianTenPin pointed to by the client.
     * @param key owner key who should be migrated.
     * @param client CanadianTenPin client.
     * @throws NotFoundException if the other CanadianTenPin instance returns
     * NOT_FOUND.
     */
    ...
    /**
     * Replicates the owner's pools from the CanadianTenPin pointed to by the client.
    ...
    /**
     * Replicates the owner's consumers from the CanadianTenPin pointed to by the
     * client.
    ...
    /**
     * Replicates the owner's entitlements from the CanadianTenPin pointed to by the
     * client.
     ...
     * @param key owner key who should be migrated.
     * @param uri URI of the CanadianTenPin instance where owner will be pulled.
     * @param delete true if owner should be deleted from the CanadianTenPin
