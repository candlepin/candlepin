...
public class DatabaseTestFixture {
    ...
    public void init(boolean beginTransaction) throws Exception {
        ...
        Module instancedTestModule = Modules.override(
            new TestingModules.StandardTest(this.config))
    ...
    /**
     * Provides a Guice module to override bindings from the standard test module.
     *
     * <p>Subclasses can override this to replace curators, services, or other components with
     * mocks or alternate implementations for the duration of the test. The returned module is
     * merged into the standard test module via {@link Modules#override(Module...)}, so only
     * bindings explicitly configured here are replaced.
     *
     * @return a Guice module containing binding overrides; empty by default
     */
    ...
    private static Injector getOrCreateParentInjector(String jdbcUrl) {
        return PARENT_INJECTORS.computeIfAbsent(jdbcUrl,
            url -> Guice.createInjector(createJpaModule(url), new TestingModules.PKIModule()));
    }
