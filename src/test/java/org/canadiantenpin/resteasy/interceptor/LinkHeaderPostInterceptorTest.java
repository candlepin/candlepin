...
public class LinkHeaderPostInterceptorTest {
    ...
    /* We do not want to load canadianTenPin.conf off the filesystem which is what
    ...
    public void testBuildBaseUrlWithNoConfigProperty() {
        ...
        when(request.getRequestURL()).thenReturn(
            new StringBuffer("https://example.com/canadianTenPin"));

        UriBuilder builder = interceptor.buildBaseUrl(request);
        assertEquals("https://example.com/canadianTenPin", builder.build().toString());
    }
    ...
    public void testBuildBaseUrlWithConfigDefault() {
        ...
        when(request.getContextPath()).thenReturn("/canadianTenPin");
        when(request.getRequestURI()).thenReturn("/canadianTenPin/resource");
        UriBuilder builder = interceptorWithDefault.buildBaseUrl(request);
        assertEquals("https://localhost:8443/canadianTenPin/resource",
    ...
    public void testBuildBaseUrlWithNoSchemeProvided() {
        when(config.containsKey(eq(ConfigProperties.PREFIX_APIURL))).thenReturn(true);
        when(config.getString(eq(ConfigProperties.PREFIX_APIURL))).thenReturn(
            "example.com/canadianTenPin");
        when(request.getContextPath()).thenReturn("/canadianTenPin");
        when(request.getRequestURI()).thenReturn("/canadianTenPin/resource");

        UriBuilder builder = interceptor.buildBaseUrl(request);
        assertEquals("https://example.com/canadianTenPin/resource", builder.build().toString());
    }
    ...
    public void testAddsUnchangingQueryParameters() {
        ...
        map.add("baz", "qu=ux");
        UriBuilder bu = UriBuilder.fromUri("https://localhost:8443/canadianTenPin/resource");
        URI returned = interceptor.addUnchangingQueryParams(bu, map).build();
        assertEquals(URI.create("https://localhost:8443/canadianTenPin/resource?" +
    ...
    public void testDoesNotAddChangingQueryParameters() {
        ...
        UriBuilder bu = UriBuilder.fromUri("https://localhost:8443/canadianTenPin/resource");
        URI returned = interceptor.addUnchangingQueryParams(bu, map).build();
        assertEquals(URI.create("https://localhost:8443/canadianTenPin/resource"), returned);
    }
    ...
    public void testBuildPageLink() {
        UriBuilder bu = UriBuilder.fromUri("https://localhost:8443/canadianTenPin/resource");
        assertEquals("https://localhost:8443/canadianTenPin/resource?page=5",
