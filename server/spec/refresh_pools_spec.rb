...
describe 'Refresh Pools' do
  ...
  it 'detects changes in branding' do
    ...
    # Add an additional branding to the upstream subscription than the one we had initially...
    sub.branding = [b1, b2]
    update_upstream_subscription(sub.id, sub)
  ...
  it 'regenerates entitlements when branding for a product of an entitled pool changes' do
    ...
    # Update the name of the branding of the upstream subscription...
    sub.branding[0].name = 'new branding name!'
    update_upstream_subscription(sub.id, sub)
