...
describe 'Entitlement Certificate V3' do
  ...
  it 'verify branding info is correct in json blob' do
    ...
    # Adding branding to marketing product, as should be done in standalone mode
    mkt_product = create_product("555", "marketing_product_name", :branding => branding)
    expect(mkt_product.branding.size).to eq(1)

    now = DateTime.now
    # Adding branding to upstream subscription, as should be done in hosted mode (branding should be ignored
    # here when creating a pool in standalone mode).
    pool = create_pool_and_subscription(@owner['key'], mkt_product.id, 10, [eng_product.id],
                                        '12345', '6789', 'order1', now - 10, now + 365, false, :branding => branding)
