...
describe 'Export', :serial => true do
  ...
  RSpec.shared_examples "exporter" do |async|

    before(:all) do

      # Decide the import method to use, async or synchronous
      if async
        @exporter = AsyncStandardExporter.new
      else
    ...
    it 'should support only exporting certificates' do
      ...
      # All 6 should be there, despite one cert is for a virt product
      exported_entitlement_certs.size.should == 6
    ...
    it 'should export products' do
      ...
      # If we're not running in hosted mode, verify the certs exist
      # In hosted mode, the adapters do not return or generate certs
      if !is_hosted?
        expected_certs = 0
        ...
        expect(certs.length).to eq(expected_certs)
        expect(File.exists?(File.join(prod_dir, "#{@exporter.products[:derived_provided_prod].id}.pem"))).to eq(true)
      end
    end
  ...
  describe "Async Export Tests" do
    it_should_behave_like "exporter", true
  end

  describe "Sync Export Tests" do
    it_should_behave_like "exporter", false
  end

end

class AsyncExportFailure < Exception
  attr_reader :data
