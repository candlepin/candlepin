...
public class SystemPurposeComplianceRules {
    ...
    public SystemPurposeComplianceStatus getStatus(Consumer consumer,
        ...
        for (Entitlement entitlement : entitlements) {
            ...
            Set<String> unsatisfiedAddons = consumer.getAddOns();
            ...
            for (Product product : products) {
                if (StringUtils.isNotEmpty(unsatisfedRole) && product.hasAttribute("ROLE")) {
                    List<String> roles =
                        Arrays.asList(product.getAttributeValue("ROLE").split("\\s*," + "\\s*"));
                ...
                if (CollectionUtils.isNotEmpty(unsatisfiedAddons) && product.hasAttribute("ADDONS")) {
                    List<String> addOns =
                        Arrays.asList(product.getAttributeValue("ADDONS").split("\\s*," + "\\s*"));
                ...
                if (product.hasAttribute("SERVICE_LEVEL")) {
                    String sla = product.getAttributeValue("SERVICE_LEVEL");
                ...
                if (product.hasAttribute("USAGE")) {
                    String usage = product.getAttributeValue("USAGE");
        ...
        if (StringUtils.isNotEmpty(consumer.getUsage()) && status.getCompliantUsage().isEmpty()) {
            status.setNonCompliantUsage(consumer.getServiceLevel());
        }
