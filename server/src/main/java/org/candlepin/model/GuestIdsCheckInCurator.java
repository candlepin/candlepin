 ...
 */
public class GuestIdsCheckInCurator extends AbstractHibernateCurator<CheckIn> {

    private static Logger log = LoggerFactory.getLogger(GuestIdsCheckInCurator.class);

    public static final int DELETE_BATCH_SIZE = 30000;

    public GuestIdsCheckInCurator() {
        super(CheckIn.class);
    }
    ...
    public int cleanupOldCheckIns() {
        ...
        String hql = "SELECT cc1.id FROM GuestIdsCheckIn cc1 WHERE cc1.updated != (" +
                "SELECT MAX(cc2.updated) " +
                "FROM GuestIdsCheckIn cc2 " +
        ...
        int totalDeleted = 0;
        int totalQueries = 1;
        String deleteHql = "DELETE from GuestIdsCheckIn where id in (:deleteIds)";
        Query deleteQuery = currentSession().createQuery(deleteHql);
        while (checkInIds.size() > 0) {
            log.debug("Found {} guest ids check-in IDs to be deleted.", checkInIds.size());
        ...
        log.info("Deleted a total of {} consumer guest ids check-in times with {} queries.",
