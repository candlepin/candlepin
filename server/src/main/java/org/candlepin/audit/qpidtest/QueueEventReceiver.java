 ...
 */
public class QueueEventReceiver {
    private static Logger log = LoggerFactory.getLogger(QueueEventReceiver.class);
    ...
    @Inject
    public QueueEventReceiver(Configuration config)
        throws Exception {
...
//        AMQAnyDestination dest = new AMQAnyDestination("event");
        AMQQueue q = new AMQQueue("localhost:event");
//        AMQTopic topic = new AMQTopic("event");
        MessageConsumer consumer = sess.createConsumer(q);
        
//        QueueBrowser browser = sess.createBrowser(q);
        try {
            Thread.sleep(2000);
...
//        System.out.println("Trying to browse");
        while (true) {
            TextMessage m = (TextMessage) consumer.receive();
            m.acknowledge();
            System.out.println(m);
//            Enumeration<Object> e = browser.getEnumeration();
//            while (e.hasMoreElements()) {
//                e.nextElement();
//                System.out.println("Found a message in a queue! "  );
            }
//        }
    }
    ...
    private class QpidConnectionThread extends Thread {
        ...
        public void run() {
            try {
                ...
                sleep(1000);
                QueueEventReceiver.this.init(config);
            }
            catch (Exception e) {
