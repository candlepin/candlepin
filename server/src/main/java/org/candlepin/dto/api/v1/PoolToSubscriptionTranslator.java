...
public class PoolToSubscriptionTranslator implements ObjectTranslator<Pool, SubscriptionDTO> {
    ...
    @Inject ProductCurator productCurator;

    public PoolToSubscriptionTranslator(ProductCurator productCurator) {
    ...
    public SubscriptionDTO populate(
        ...
        if (translator != null) {
            ...
            Set<Product> products = productCurator.getPoolProvidedProductsCached(source);
            if (products != null) {
                Set<ProductDTO> providedProduct = new HashSet<>();

                for (Product product : products) {
                    providedProduct.add(translateObject(translator,
                        translator.getTranslator(Product.class, ProductDTO.class), product));
                }

                dest.setProvidedProducts(providedProduct);
            }
            else {
                dest.setProvidedProducts(null);
            }

            products = productCurator.getPoolDerivedProvidedProductsCached(source);
            if (products != null) {
                Set<ProductDTO> dProvidedProduct = new HashSet<>();

                for (Product product : products) {
                    dProvidedProduct.add(translateObject(translator,
                        translator.getTranslator(Product.class, ProductDTO.class), product));
                }

                dest.setDerivedProvidedProducts(dProvidedProduct);
            }
            else {
                dest.setDerivedProvidedProducts(null);
            }
