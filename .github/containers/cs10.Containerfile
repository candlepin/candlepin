...
RUN dnf -y --setopt install_weak_deps=False update && \
    dnf -y --setopt install_weak_deps=False install temurin-17-jdk openssl wget unzip && \
    dnf clean all

# Prepare Tomcat
COPY apache-tomcat-10.1.36.tar.gz /tmp/
RUN tar xzf /tmp/apache-tomcat-10.1.36.tar.gz -C /tmp && \
    mkdir /opt/tomcat && \
    mv /tmp/apache-tomcat-10.1.36/* /opt/tomcat/
...
RUN dnf -y update && \
    ...
    dnf install -y temurin-17-jdk openssl initscripts && \
    dnf clean all
