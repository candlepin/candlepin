...
RUN dnf -y --setopt install_weak_deps=False update && \
    dnf -y --setopt install_weak_deps=False install java-17-openjdk-devel openssl && \
    dnf clean all
...
RUN dnf -y update && \
    ...
    dnf install -y java-17-openjdk-headless openssl initscripts wget tar unzip && \
    dnf clean all
...
RUN ls /opt/tomcat/webapps/candlepin
