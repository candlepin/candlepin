...
def fetch_product_uuids(db, org_id):
    ...
    product_uuid_query = 'SELECT op.uuid FROM cp_products op WHERE op.namespace IN (SELECT account FROM cp_owner WHERE id=%s UNION SELECT \'\') ' + \
        'UNION ' + \
...
class ContentManager(ModelManager):
    ...
    def do_export(self):
        ...
        content_uuid_query1 = 'SELECT oc.uuid FROM cp_contents oc WHERE oc.namespace IN (SELECT account FROM cp_owner WHERE id=%s UNION SELECT \'\')'
