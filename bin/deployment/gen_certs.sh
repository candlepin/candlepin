...
CERT_OUT_RSA="${CP_CERT_HOME}/candlepin-ca-rsa.crt"
KEY_OUT_RSA="${CP_CERT_HOME}/candlepin-ca-rsa.key"
CERT_OUT_MLDSA="${CP_CERT_HOME}/candlepin-ca-mldsa.crt"
KEY_OUT_MLDSA="${CP_CERT_HOME}/candlepin-ca-mldsa.key"
...
if [ -z $CA_CERT ]; then
    ...
    if [ $GEN_MLDSA -eq 1 ]; then
        ...
        # Create legacy symlinks for backward compatibility (point to ML-DSA)
        echo ""
        echo "Creating legacy symlinks (pointing to ML-DSA certificates)..."
        ln -sf "$(basename $CERT_OUT_MLDSA)" "$CERT_OUT"
        ln -sf "$(basename $KEY_OUT_MLDSA)" "$KEY_OUT"
    else
    ...
    trap "rm -f ${TMP_CSR_FILE_RSA}" EXIT
