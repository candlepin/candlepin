...
class CandlepinConnectorEditorV3(AbstractBaseEditor):
    ...
    def _build_node(self):
        ...
        #         protocols="TLSv1.3"
        #         sslProtocol="TLS"
        #         truststoreFile="/etc/candlepin/certs/candlepin-truststore.p12"
        #         truststorePassword="password"
        #         truststoreType="PKCS12">
        #
        #         <Certificate
        #                certificateKeystoreFile="/etc/candlepin/certs/candlepin-keystore.p12"
        ...
        self._add_attributes(ssl_host_config, [
            ...
            # For the time being, TLSv1 needs to stay enabled in Satellite deployments to support
            # existing python-rhsm based clients (RHEL5).
            # Using TLSv1.2 instead of TLSv1.3 to avoid JSSE PHA issues with optional client auth
            # ML-DSA certificates work fine with TLS 1.2
            ("protocols", "TLSv1.2"),
            ("sslProtocol", "TLS")
            # Truststore disabled temporarily - Java PKIXBuilderParameters doesn't accept ML-DSA certs as trust anchors
            # ('truststoreFile', '/etc/candlepin/certs/candlepin-truststore.p12'),
            # ('truststorePassword', 'password'),
            # ('truststoreType', 'PKCS12')
        ])
        ...
        self._add_attributes(certificate, [
            # Use PKCS12 keystore - requires BouncyCastle providers for ML-DSA
            ('certificateKeystoreFile', '/etc/candlepin/certs/candlepin-keystore.p12'),
            ('certificateKeystorePassword', 'password'),
            ('certificateKeystoreType', 'PKCS12')
        ])
