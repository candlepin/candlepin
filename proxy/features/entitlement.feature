Feature: Consume an Entitlement
    ...
    Scenario: Entitlement is Consumed
        ...
        When I Consume an Entitlement for the "RHN Satellite Virtulization Host" Product
        Then I Have 1 Entitlement
        
    Scenario: Entitlement from a Pool is Consumed 
        Given I am a Consumer "random_box"
        When I Consume an Entitlement for the "RHN Satellite Virtulization Host" Pool
        Then I Have 1 Entitlement

    Scenario: Multiple Entitlements are Consumed
        Given I am a Consumer "consumer"
        When I Consume an Entitlement for the "RHN Satellite Virtulization Host" Product 
        And I Consume an Entitlement for the "RHN Satellite Monitoring" Product
    ...
    Scenario: Single Entitlement has the correct productId
        ...
        Then I Have an Entitlement for the "RHN Satellite Monitoring" Product

    Scenario: Single Entitlement from a Pool has the correct productId 
        Given I am a Consumer "random_box"
        When I Consume an Entitlement for the "RHN Satellite Monitoring" Pool
        Then I Have an Entitlement for the "RHN Satellite Monitoring" Product
    ...
    Scenario: Multiple Entitlements have correct productIds
        ...
        Then I Have an Entitlement for the "RHN Satellite Monitoring" Product
        And I Have an Entitlement for the "RHN Satellite Virtualization Host" Product

    Scenario: A Consumer has No Entitlements After Unregistering
        Given I am a Consumer "some_machine"
        When I Consume an Entitlement for the "RHN Satellite Virtualization Host" Product
    ...
    Scenario: Consuming the same entitlement by product twice is not allowed
        Given I am a Consumer "random_box"
        When I Consume an Entitlement for the "RHN Satellite Virtualization Host" Product
        And I try to consume an Entitlement for the "RHN Satellite Virtualization Host" Product again
        Then I recieve an http forbidden response

    Scenario: Consuming the same entitlement by pool twice is not allowed
        Given I am a Consumer "random_box"
        When I Consume an Entitlement for the "RHN Satellite Virtualization Host" Pool
        And I try to consume an Entitlement for the "RHN Satellite Virtualization Host" Pool again
        Then I recieve an http forbidden response
