...
product_sql_template = """insert into cp_product (id, arch, hash, label,name, variant, version)
                values ('%(product_id)s',
                         'ALL',
                          %(product_hash)s,
                         '%(product_label)s',
...
subscription_sql_template = """insert into cp_subscription (id,enddate, modified, productid,
                             quantity, startdate, owner_id)
                values (nextval('seq_subscription'),
                        '2010-07-13 00:00:00',
                        '2007-07-13 00:00:00',
                        '%(product_name)s',
                        1984,
...
# NOTE: the content type is currently hardcode to YUM, the data mode
# supports different types, but the import script does not yet
content_sql_template = """insert into cp_content (id, type, contenturl, enabled,
                        gpgurl,hash, label,
                         name, vendor)
            values (nextval('seq_content'),
		   'yum',
                   '%(content_url)s',
                   '%(content_enabled)s',
                    '%(gpg_url)s',
...
product_content_sql_template = """insert into cp_product_content (cp_product_id, content_id)
           values ('%(product_id)s', (select id from cp_content where label='%(content_label)s'));
"""

pool_sql_template = """
insert into cp_pool (id, activesubscription, consumed,
                     enddate, productid, quantity,
                     startdate, subscriptionid, owner_id)
           values (nextval('seq_pool'),
                   't',
                    0,
                    '2010-07-13 00:00:00',
                    '%(product_id)s',
                    1984,
                    '2007-07-13 00:00:00',
                    (select id from cp_subscription where productid='%(product_id)s'),
                     1);
...
#        print pc
        ...
        p = pc.getProduct()
#        print pc.getProducts()
#        print pc.getContentEntitlements()

        ec = EntitlementCertificate()
        ec.read(path)

        ces = ec.getContentEntitlements()
        
        product_strings =  {'product_id': p.getName(), 
                            'product_hash': p.getHash(), 
                            'product_label': p.getName(), 
                            'product_name': p.getName(),
                            'product_variant': p.getVariant()}

        print product_sql_template % product_strings

        print subscription_sql_template % product_strings

        for content in ces:
            content_strings = {'content_url':content.getUrl(),
                               'content_enabled': content.getEnabled(),
                               'gpg_url': content.getGpg(),
                               'content_hash': content.getHash(),
                               'content_label': content.getLabel(),
                               'content_name': content.getName(),
                               'content_vendor':content.getVendor(),
                               'product_id': p.getName()}
            print content_sql_template % content_strings

            print product_content_sql_template % content_strings

        print pool_sql_template % product_strings
