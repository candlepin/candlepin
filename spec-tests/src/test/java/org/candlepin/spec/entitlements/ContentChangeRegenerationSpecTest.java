...
class ContentChangeRegenerationSpecTest {
    ...
    private enum ApiChange {
        ADD_CONTENT, ADD_CONTENT_BATCH, REMOVE_CONTENT, REMOVE_CONTENT_BATCH, UPDATE_PRODUCT,
        UPDATE_CONTENT, UPSERT_CONTENT
    }
    ...
    void shouldRegenerateCertificatesAfterApiContentChanges(ProductLocation location, ApiChange change) {
        ...
        boolean removingContent = change == ApiChange.REMOVE_CONTENT ||
            change == ApiChange.REMOVE_CONTENT_BATCH;
        ContentDTO changed;
        if (removingContent) {
            changed = original;
        }
        else if (updatingContent) {
            changed = Contents.copy(original).label(original.getLabel() + "-updated");
        }
        else {
            changed = admin.ownerContent().createContent(owner.getKey(), Contents.random());
        }
        ...
        if (removingContent) {
            assertDoesNotContainContent(regenerated, target, changed);
        }
        else {
        ...
        if (location == ProductLocation.DIRECT || location == ProductLocation.PROVIDED) {
            ...
            if (removingContent) {
                assertDoesNotContainContent(systemCertificate, target, changed);
            }
            else {
    ...
    private void applyContentChange(ApiClient admin, OwnerDTO owner, ProductDTO target, ContentDTO content,
        ...
        switch (change) {
            ...
            case REMOVE_CONTENT -> admin.ownerProducts().removeContentFromProduct(owner.getKey(), target.getId(),
                content.getId());
            case REMOVE_CONTENT_BATCH -> admin.ownerProducts().removeContentsFromProduct(owner.getKey(),
                target.getId(), List.of(content.getId()));
    ...
    private void assertContainsContent(CertificateDTO certificate, ProductDTO product, ContentDTO content) {
        assertThat(findContent(certificate, product, content)).singleElement()
            .returns(content.getLabel(), node -> node.path("label").asText());
    }

    private void assertDoesNotContainContent(CertificateDTO certificate, ProductDTO product, ContentDTO content) {
        assertThat(findContent(certificate, product, content)).isEmpty();
    }

    private List<JsonNode> findContent(CertificateDTO certificate, ProductDTO product, ContentDTO content) {
        JsonNode body = CertificateUtil.decodeAndUncompressCertificate(certificate.getCert(), ApiClient.MAPPER);
        assertThat(body).isNotNull();
        return StreamSupport.stream(body.path("products").spliterator(), false)
